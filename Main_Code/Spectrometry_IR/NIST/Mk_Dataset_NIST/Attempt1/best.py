...
# Create tables for molecules and spectra
cursor.execute('''
...
for filename in os.listdir(dir_path):
    if filename.endswith('.jdx') or filename.endswith('.jsx'):
        ...
        if title == '' and names == '':
            ...
            continue
        cursor.execute('''
            INSERT INTO molecules (title, names) VALUES (?, ?)
        ''', (title, names))
        molecule_id = cursor.lastrowid
        data_to_insert = [(molecule_id, x, y) for x, y in xydata]
        cursor.executemany('''
            INSERT INTO spectra (molecule_id, x, y) VALUES (?, ?, ?)
        ''', data_to_insert)
        conn.commit()
        print(f"Inserted data for molecule '{title}' into database.")
